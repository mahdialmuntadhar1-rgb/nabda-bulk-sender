...
import type { OptOutRecord } from '../types.js';

const OPTOUT_FILE = './opt-outs.json';

export function loadOptOuts(): Set<string> {
  if (!existsSync(OPTOUT_FILE)) return new Set();
  try {
    const content = readFileSync(OPTOUT_FILE, 'utf-8');
    const records: OptOutRecord[] = JSON.parse(content);
...
export function addOptOut(phone: string, reason: string): void {
  const existing = loadOptOutRecords();
  const normalized = phone.replace(/^\+/, '');
  if (existing.some(r => r.phone === normalized)) return;
  existing.push({
    phone: normalized,
...
function loadOptOutRecords(): OptOutRecord[] {
  if (!existsSync(OPTOUT_FILE)) return [];
  try {
