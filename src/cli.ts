...
program
  ...
  .action(async (options: { csv: string; template: string; dryRun: boolean; limit?: number; concurrency: number; batchSize: number; batchDelayMs: number; resume: boolean; log: string }) => {
    try {
      if (!options.dryRun) validateConfig();
